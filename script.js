...
const patterns = {
    contactName: /^[A-Za-z\s]{1,25}$/,
    contactNo: /^09([0-9])[0-9]{8}$/
};
...
searchCont.addEventListener('keyup', function (e) {
    ...
    if (radio == 0) { // Searching by Name
        ...
        for (i = 0; i < contacts.length; i++) {
            ...
            infoName = infoName.textContent.toLowerCase();
...
Array.from(contRow).forEach(function (roww) {
    roww.addEventListener('click', function (e) {
        ...
        phonedel = targetCont.children[1].textContent;
...
function exists(p) {
    return contacts.some(contact => contact.phoneNo === p);
}
